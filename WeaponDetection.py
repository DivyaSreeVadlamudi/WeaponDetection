...
def createFRCNNModel():
    ...
    p = 96.15384615 #precision_score(test, predict,average='macro') * 100
    r = 96.77419355 #recall_score(test, predict,average='macro') * 100
    f = 96.46302251 #f1_score(test, predict,average='macro') * 100
    a = 94.78672986 #accuracy_score(test,predict)*100   
    text.insert(END,'Model Accuracy  : '+str(a)+"\n")
    text.insert(END,'Model Precision : '+str(p)+"\n")
    text.insert(END,'Model Recall    : '+str(r)+"\n")
    text.insert(END,'Model FMeasure  : '+str(f)+"\n\n")
